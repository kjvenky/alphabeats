namespace :cron do
  ...
  membership_cron_hash ={ expired_membership_user_list: [], membership_fee_renewal_user_list: [], thirty_day_reminder_user_list: [], ten_day_reminder_user_list: [] } 
  albumaddon_cron_hash ={ expired_albumaddon_album_list: [], albumaddon_fee_renewal_album_list: [], thirty_day_reminder_album_list: [], ten_day_reminder_album_list: [] } 
  ...
  task :albumaddon => :environment do
    AlbumAddon.all.each do |albumaddon|
      ...
      #Handles renewal and deduction
      if albumaddon.album.musician.wallet.amount >= 20 && Date.today >albumaddon.next_renewal_date >= Date.today -7
        ActiveRecord::Base.transaction do

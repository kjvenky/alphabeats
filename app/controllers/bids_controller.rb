...
private
  ...
  def make_bid_offer_trade(bid)
    ...
    offer = Offer.where(song_id: bid.song_id, open_status: 1).order(offer_price: :asc).first
    ...
    begin
      ActiveRecord::Base.transaction do
        ...
        TradeLog.create!(seller_id: offer.user.id, buyer_id: current_user.id, song_id: bid.song.id, share: min_share_traded.share , amount: amount_paid , transaction_log_id: transactionlog.id )

        #update_wallet
        bid_new_wallet_amount = current_user.wallet.amount - amount_paid
        current_user.wallet.update_attributes!(amount: bid_new_wallet_amount)
        ...
        if !offer.song.shareholders.find_by(user_id: current_user.id)
            # bidder_shareholder_account = offer.song.shareholders.create!(user_id: current_user.id, share: 0)
          Shareholder.create!(song_id: offer.song.id, user_id: current_user.id, share: min_share_traded.share)
        else
          bidder_shareholder_account = offer.song.shareholders.find_by(user_id: current_user.id)
